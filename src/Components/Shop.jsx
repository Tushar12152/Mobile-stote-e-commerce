...
const Shop = () => {
  ...
  const categories = [
    { name: "All", filter: products },
    { name: "Apple", filter: filterByCategory('apple') },
    { name: "Samsung", filter: filterByCategory('samsung') },
    { name: "VIVO", filter: filterByCategory('vivo') },
    { name: "Infinix", filter: filterByCategory('infinix') },
    { name: "Realme", filter: filterByCategory('realme') },
    { name: "Redme", filter: filterByCategory('redme') },
    { name: "Howaway", filter: filterByCategory('howaya') },
    { name: "OPPO", filter: filterByCategory('oppo') },
  ];
        ...
        <div className="flex justify-between items-center">
          <TabList className="flex gap-6 mb-10 cursor-pointer">
            {categories.map((category, index) => (
              <Tab key={index} className="cursor-pointer p-3">
                {category.name}
              </Tab>
            ))}
          </TabList>
        ...
        {categories.map((category, index) => (
          <TabPanel key={index}>
            ...
            <div className="mt-10 grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-3">
              {category.filter.length > 0 ? (
                category.filter.map((product) => (
                  <ProductCart key={product._id} product={product} />
                ))
              ) : (
                <h1 className="font-bold text-2xl text-red-500">Not available right now</h1>
              )}
            </div>
          </TabPanel>
        ))}
